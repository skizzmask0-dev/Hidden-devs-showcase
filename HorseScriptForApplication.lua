...
-- this scrpit handles the race sequense on the cilent side, the mountign cutscene, screen flahs transtions,
-- speedline vfx and the chase camera that folows behind the horse while ur racing
...
local function linesTick()
	...
	local ratio = plr:GetAttribute("RaceSpeedRatio")
...
local function camOn()
	...
	camHeartbeat = RunService.RenderStepped:Connect(function()
		...
		local ratio = math.clamp(plr:GetAttribute("RaceSpeedRatio"), 0, 1)
		local now = os.clock()
...
local function mountCutscene(cb)
	...
	task.delay(Config.CUTSCENE_DURATION, function()
		if not isRacing then return end -- race got cancled mid cutscene
...
RaceEnded.OnClientEvent:Connect(function()
	...
	letterboxToggle(false)
	doFlash(Config.SCREEN_FLASH_COLOR, Config.RESPAWN_FLASH_ALPHA, Config.RESPAWN_FLASH_FADE_TIME) -- covers the hard cam snap back
	camOff()
	linesOff()
end)
